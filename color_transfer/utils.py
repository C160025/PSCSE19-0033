...
def cx_rgb2lab(image_rgb, log10):
    ...
    # equation 4 (FYP figure 2.6) to to convert RGB space to LMS space
    rgb2lms_eq2 = [[0.3811, 0.5783, 0.0402],
                   [0.1967, 0.7244, 0.0782],
                   [0.0241, 0.1288, 0.8444]]

    # left of equation 6 (FYP figure 2.8) to convert LMS space to lab space
    lms2lab_eq1 = [[1 / np.sqrt(3), 0.0000, 0.0000],
                   [0.0000, 1 / np.sqrt(6), 0.0000],
                   [0.0000, 0.0000, 1 / np.sqrt(2)]]

    # right of equation 6 (FYP figure 2.8) to convert LMS space to lab space
    lms2lab_eq2 = [[1.0000, 1.0000, 1.0000],
                   [1.0000, 1.0000, -2.0000],
                   [1.0000, -1.0000, 0.0000]]

    # dot product of equation 6 (FYP figure 2.8) to convert LMS space to lab space
    ...
    # equation 5 (FYP figure 2.7) to eliminate the skew
    if log10:
...
def cx_lab2rgb(image_lab, power10):
    ...
    """
    # left of equation 8 (FYP figure 2.11) to convert lab space to LMS space
    lab2lms_eq1 = [[1.0000, 1.0000, 1.0000],
                   [1.0000, 1.0000, -1.0000],
                   [1.0000, -2.0000, 0.0000]]

    # right of equation 8 (FYP figure 2.11) to convert lab space to LMS space
    lab2lms_eq2 = [[np.sqrt(3)/3, 0.0000, 0.0000],
                   [0.0000, np.sqrt(6)/6, 0.0000],
                   [0.0000, 0.0000, np.sqrt(2)/2]]

    # dot product of equation 8 (FYP figure 2.11) to convert lab space to LMS space
    lab2lms_eq = np.matmul(lab2lms_eq1, lab2lms_eq2)

    # equation 9 (FYP figure 2.13) to convert LMS space to RGB space
    ...
    # (FYP figure 2.12) raise back to linear space
    if power10:
        L = np.power(10, L)
        M = np.power(10, M)
        S = np.power(10, S)
